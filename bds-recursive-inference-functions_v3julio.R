...
bds_recur_stchg<-function(xserie,N0= 50, m=6, lapso=1, type="OLS-CUSUM", ... ){
  require(tseries)
  require(zoo)
  require(dplyr)
  require(ggplot2)
  ...
  for (i in seq(from=N0,to=length(xserie),by = lapso)){
    j=j+1
    cat(paste0(j, " de ", round((length(xserie)-N0)/lapso), " \n"))
    xtemp<-xserie[1:i]
    xsigtemp<-sd(xtemp)*0.5
    bds<-bds.test(xtemp,m=m,eps=xsigtemp)
    bdsout<-rbind.data.frame(bdsout, c(i, as.Date(index(xserie[i])),bds$statistic[1:m-1], bds$p.value[1:m-1]))
  }    
  names(bdsout)<-c("indice","fecha", paste0("m",2:m), paste0("p-value",2:m))  
...
#    print(g_bdsD)
  ...
  testestbr<-efp(modelo,type = "OLS-CUSUM", data = bdsout_st) # este utiliza ls residuos - detectar cambios en los residuos del modelo 
  plot(testestbr)
